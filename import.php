...
/**
 * Ambil data staging 1 batch, sudah digabung dengan data karyawan+shift,
 * dan sudah dihitung jam masuk/pulang + status. Dipakai bareng oleh
 * halaman preview & proses commit supaya hasilnya selalu konsisten.
 */
function ambil_data_preview(PDO $pdo, string $batch_id): array
{
    $stmt = $pdo->prepare(
        "SELECT s.*, e.nama AS nama_karyawan, e.nik, sh.jam_masuk AS shift_masuk, sh.jam_pulang AS shift_pulang,
                sh.toleransi_menit, sh.is_lembur, d.nama_departemen
    ...
    $hasil = [];
    foreach ($rows as $r) {
        $jam_list = $r['jam_raw'] !== null && $r['jam_raw'] !== '' ? explode(',', $r['jam_raw']) : [];
        [$masuk, $pulang, $perlu_tinjau] = tebak_masuk_pulang($jam_list, $r['shift_masuk'], $r['shift_pulang']);

        if ($r['shift_masuk']) {
            [$status, $menit_telat] = hitung_status($masuk, $r['shift_masuk'], (int) $r['toleransi_menit']);
            if ($status === 'hadir' && $r['is_lembur']) {
                $status = 'lembur';
            }
        } else {
...
if ($step === 'commit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($data as $row) {
        ...
        try {
            $pdo->prepare(
                "INSERT INTO attendance_daily (employee_id, tanggal, jam_masuk_aktual, jam_pulang_aktual, status, menit_telat, perlu_tinjau, sumber, catatan)
                 VALUES (?,?,?,?,?,?,?,'import',?)
                 ON DUPLICATE KEY UPDATE jam_masuk_aktual=VALUES(jam_masuk_aktual), jam_pulang_aktual=VALUES(jam_pulang_aktual),
                    status=VALUES(status), menit_telat=VALUES(menit_telat), perlu_tinjau=VALUES(perlu_tinjau), sumber='import', catatan=VALUES(catatan)"
            )->execute([
                $row['employee_id'], $row['tanggal'], $row['jam_masuk'], $row['jam_pulang'],
                $row['status'], $row['menit_telat'], $row['perlu_tinjau'] ? 1 : 0,
                $row['perlu_tinjau'] ? 'Hanya 1 kali scan, jam ditebak otomatis — mohon dicek.' : null,
            ]);
            ...
            $ada = $cek->fetch();
            $catatan = $row['perlu_tinjau'] ? 'Hanya 1 kali scan, jam ditebak otomatis — mohon dicek.' : null;
            if ($ada) {
                $pdo->prepare("UPDATE attendance_daily SET jam_masuk_aktual=?, jam_pulang_aktual=?, status=?, menit_telat=?, perlu_tinjau=?, sumber='import', catatan=? WHERE id=?")
                    ->execute([$row['jam_masuk'], $row['jam_pulang'], $row['status'], $row['menit_telat'], $row['perlu_tinjau'] ? 1 : 0, $catatan, $ada['id']]);
            } else {
                $pdo->prepare("INSERT INTO attendance_daily (employee_id, tanggal, jam_masuk_aktual, jam_pulang_aktual, status, menit_telat, perlu_tinjau, sumber, catatan) VALUES (?,?,?,?,?,?,?,'import',?)")
                    ->execute([$row['employee_id'], $row['tanggal'], $row['jam_masuk'], $row['jam_pulang'], $row['status'], $row['menit_telat'], $row['perlu_tinjau'] ? 1 : 0, $catatan]);
            }
        }
...
<header class="page__header">
  <div class="page__headline">
    ...
    <p class="page__description">Unggah file laporan absensi (.xls) hasil export dari mesin fingerprint lewat flashdisk.</p>
  </div>
</header>
    ...
    <section class="page__section">
      <div class="grid grid-cols-12 gap-4">
        <div class="col-span-4">
          <div class="card card--stat"><div class="card__body">
            <div class="stat"><div class="stat__value"><?= $ringkasan_preview['total'] ?></div><div class="stat__label text-eyebrow">Total Baris Data</div></div>
          </div></div>
        </div>
        <div class="col-span-4">
          <div class="card card--stat"><div class="card__body">
            <div class="stat"><div class="stat__value"><?= $ringkasan_preview['karyawan'] ?></div><div class="stat__label text-eyebrow">Karyawan Terlibat</div></div>
          </div></div>
        </div>
        <div class="col-span-4">
          <div class="card card--stat"><div class="card__body">
            <div class="stat"><div class="stat__value"><?= $ringkasan_preview['tinjau'] ?></div><div class="stat__label text-eyebrow">Perlu Ditinjau</div></div>
          </div></div>
        </div>
      </div>
    </section>
      ...
      <div class="card">
        <div class="card__header">
          ...
          <div class="card__action">
            <a href="?step=preview<?= empty($_GET['hanya_tinjau']) ? '&hanya_tinjau=1' : '' ?>" class="button button--sm button--ghost button--neutral">
              <?= empty($_GET['hanya_tinjau']) ? 'Tampilkan yang perlu ditinjau saja' : 'Tampilkan semua' ?>
            </a>
          </div>
        </div>
        <div class="table-responsive" style="max-height: 500px; overflow-y: auto;">
          <table class="table table--hover table--align-middle">
            <thead class="table__head--alt">
              <tr>
                <th>Tanggal</th><th>Karyawan</th><th>Dept</th><th>Masuk</th><th>Pulang</th><th>Status</th><th></th>
              </tr>
            </thead>
            <tbody>
